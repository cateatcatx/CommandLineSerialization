...
{
    public readonly IValueSerializer ValueSerializer;

    public CommandLineSerializer(IValueSerializer? valueSerializer = null)
    {
        ValueSerializer = valueSerializer ?? new BuiltinValueSerializer();
    }
    ...
    {
        var objectSpecs = ValueSerializer.GetObjectSpecs(objType);
        if (objectSpecs == null)
        {
    ...
    public void Deserialize(LinkedList<string> argList,
        ...
        OnDeserialized? onDeserialized)
    {
        _DeserializeOptions(argList.First, specs, onDeserialized);
        _DeserializeArguments(argList, specs, onDeserialized);
    }
    ...
    private void _SerializeOptions(IReadOnlyList<IOption> options, LinkedList<string> argList, OnSerialized? onSerialized)
    {
        foreach (var option in options)
        {
            ...
            {
                if (ValueSerializer.SerializeNonValue(this, option.ObjType, obj))
                {
            ...
            {
                argList.AddLast($"{optionPrefix}");
                argList.AddLast(ValueSerializer.SerializeSingleValue(this, option.ObjType, obj));
            ...
            {
                foreach (string value in ValueSerializer.SerializeMultiValue(this, option.ObjType, obj))
                {
                    argList.AddLast($"{optionPrefix}");
    ...
    private void _SerializeArguments(IEnumerable<IArgument> arguments, LinkedList<string> argList, OnSerialized? onSerialized)
    {
        ...
        foreach (var argument in arguments)
        {
            ...
            if (argument.ValueType == ValueType.Single)
            {
                ...
                argList.AddLast(ValueSerializer.SerializeSingleValue(this, argument.ObjType, obj));
            ...
            {
                foreach (string value in ValueSerializer.SerializeMultiValue(this, argument.ObjType, obj))
                {
                    if (!addDemarcate)
                    {
                        argList.AddLast("--");
    ...
    private void _DeserializeOptions(LinkedListNode<string>? node, ISpecs specs, OnDeserialized? onDeserialized)
    {
    ...
    private void _DeserializeArguments(LinkedList<string> argList, ISpecs specs, OnDeserialized? onDeserialized)
    {
    ...
    {
        return spec.CanHandleType(spec.ObjType) ? spec.DeserializeNonValue(this, spec.ObjType, matched) : ValueSerializer.DeserializeNonValue(this, spec.ObjType, matched);
    }

    private object? _DeserializeSingleValue(ISpec spec, string? value)
    {
        var limitValues = spec.GetLimitValues();
        if (limitValues != null)
        {
            if (value == null || !limitValues.Contains(value))
                throw new InvalidOperationException($"{spec.ValueName}'s value '{value}' is not in '{string.Join(",", limitValues)}'");
        }
            
        return spec.CanHandleType(spec.ObjType) ? spec.DeserializeSingleValue(this, spec.ObjType, value) : ValueSerializer.DeserializeSingleValue(this, spec.ObjType, value);
    }
    ...
    private object? _DeserializeMultiValue(ISpec spec, List<string> values)
    {
        ...
        return spec.CanHandleType(spec.ObjType) ? spec.DeserializeMultiValue(this, spec.ObjType, values) : ValueSerializer.DeserializeMultiValue(this, spec.ObjType, values);
    }
