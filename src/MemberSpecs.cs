...
public class MemberSpecs : ISpecs
{
    ...
    private ISpec _GenerateSpec(MemberInfo memberInfo, SpecAttribute? attr)
    {
        ...
        return attr != null ?
            ImplUtil.GenerateSpecByAttribute(attr, objType, memberInfo.Name, isOption => ImplUtil.GetDefaultValueType(isOption, objType), null) :
            new Option(memberInfo.Name, ImplUtil.GetDefaultValueType(true, objType), objType, memberInfo.Name);
    }
}